...
import { METADATA_QUERY, MetadataQuery, SEARCH_METADATA_QUERY, SearchMetadataQuery } from "../../graphql/queries/Metadata";
import { CATEGORIES_QUERY, CategoriesQuery } from "../../graphql/queries/Categories";
...
const Articles : React.FC = () => {
  const [getMetadata, metadataResponse] = useLazyQuery<MetadataQuery.Response, MetadataQuery.Variables>(METADATA_QUERY);
  const categoriesResponse = useQuery<CategoriesQuery.Response>(CATEGORIES_QUERY);
  const [search, searchResponse] = useLazyQuery<SearchMetadataQuery.Response, SearchMetadataQuery.Variables>(SEARCH_METADATA_QUERY)
  ...
  useEffect(() => {
    ...
    if (metadataResponse.data && !metadataResponse.loading){
  ...
  const addFilterCategory = (category: string) => {
    setFilterCategories(prev => new Set(prev.add(category)))
  }
  ...
  return (
    <Box w="100%">
      ...
      />
        <HStack my="2" >
          {
            ...
            categoriesResponse.data.categories.filter(c => !filterCategories.has(c)).map(category => {
              return (
                <Tag
                  key={category}
                  variant="solid"
        ...
        <Box w="100%">
          <Heading fontFamily="heading" as="h2">Posts</Heading>
          {
              ...
              <HStack>
                <Text>Filtered by: </Text>
                {
                  [...filterCategories].map((category) => {
                    return (
                      <Tag key={category} variant="solid" colorScheme="black">
                        <TagLabel>{category}</TagLabel>
                        <TagCloseButton onClick={() => removeFilterCategory(category)} />
                      </Tag>
                    )
                  })
                }
          ...
          <SimpleGrid columns={2} spacing="5" pt="5" >
            {
              searching && metadata && !metadata.length && (
                <Text>
                  Could not find any results for: <Text as="span" fontWeight="bold">{searchQuery}</Text>
                </Text>
              )
            }
            {
            ...
            }
            {
              ...
              metadata.map( post => {
                ...
                const elapsed = formatDistance(publishedDate, new Date(), { addSuffix: true });
                return (
                  <Card
                    key={id}
                    as={ReactLink}
                  ...
                  >
                    {
                      image && <Image src={image} borderRadius="lg"  h="40vh" w="100%" objectFit="cover" />
                    }
                    <Box p="5" >
                      <Heading fontFamily="heading" size="lg">{name}</Heading>
                      <HStack spacing="3" my="1" >
                        {
                          categories.map(category => {
                            return <Tag key={category} variant="solid" colorScheme="gray" >{category}</Tag>
                          })
                        }
                      </HStack>
