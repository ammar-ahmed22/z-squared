...
import { BlockType, IBlock, isRichTextContent, isImageContent, isListContent, IBlockContent, IRichText, isEquationContent } from "@z-squared/types"
import { Text, OrderedList, UnorderedList, Image, TextProps, ImageProps, ListProps, ListItem, Spinner, Box, BoxProps, ColorMode, IconButton, useColorMode, useColorModeValue } from "@chakra-ui/react"
...
type ComponentProps = TextProps | ImageProps | ListProps | CodeStyles
...
function colorModeValue<T = any>(light: T, dark: T, mode: ColorMode): T {
  if (mode === "dark") return dark;
  return light;
}

export const renderBlockContent = (content: IBlockContent[], key: string) => {
  return content.map( (item, idx) => {
    if (isRichTextContent(item)){
      // @ts-ignore
      return <Text key={`${key}-item-${idx}`} as="span">{item.plainText}</Text>
        ...
        children.map( child => {
          ...
          return (
            <ListItem key={`list-${idx}`}>
              {
                content.map( (c, cIdx) => <Text key={`${key}-list-${idx}-item-${cIdx}`} as="span">{c.plainText}</Text>)
              }
            </ListItem>
          )
        })
...
export const renderBlock = (block: IBlock, key: string, mode: ColorMode) => {
  ...
  switch (type) {
    ...
    case "bulleted_list":
      return (
        <MathJax.Provider>
          <UnorderedList {...props as ListProps} key={key} >
            {
        ...
        return (
          ...
          >
            <SyntaxHighlighter
              ...
              style={colorModeValue(atomOneLight, atomOneDark, mode)}
            >
              {richText.plainText}
            </SyntaxHighlighter>
            <IconButton 
...
export const useRenderedBlocks = (data?: IBlock[]): JSX.Element[] => {
