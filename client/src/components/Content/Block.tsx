...
import { IBlock, BlockType, IRichText, isImageContent, isRichTextContent, isEquationContent, isListContent } from "@z-squared/types";
import { 
  TextProps, 
  ImageProps, 
  ListProps,
...
const Block: React.FC<BlockProps> = ({
  ...
  const key = blockId;
  const [copyProps, setCopyProps] = useState<IconButtonProps>({ variant: "ghost", icon: (<BiCopy />), "aria-label": "copy the code block", display: "none" });
    ...
    case "bulleted_list":
      ...
      if (isListContent(bl)) {
        ...
        return (
          ...
          >
            {
              children.map((child, idx) => <RecursiveListItem data={child} blockId={key} listType={type} depth={0} key={blockId + idx}/>)
            }
          </UnorderedList>
        )
      }
    ...
    case "numbered_list":
      ...
      if (isListContent(nl)) {
        ...
        return (
          ...
          >
            {
              children.map((child, idx) => <RecursiveListItem data={child} blockId={key} listType={type} depth={1} key={blockId + idx}/>)
            }
          </OrderedList>
        )
      }
      ...
      if (isRichTextContent(richText)) {
        ...
        return (
          <Box
            ...
            pos="relative"
            onMouseEnter={() => {
              setCopyProps(prev => ({...prev, display: "inline-flex" }))
            }}
            onMouseLeave={() => {
              setCopyProps(prev => ({...prev, display: "none" }))
            }}
          >
            <SyntaxHighlighter
              language={language}
            ...
            <IconButton 
              ...
              right="1"
              // variant="ghost"
              onClick={() => handleCodeCopy(richText.plainText)}
